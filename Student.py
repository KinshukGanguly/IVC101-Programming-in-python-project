...
def Report_Card():
    ...
    for i in range(len(dfb)):
        if dfb.iat[i,0]==batch_ID:
            st=dfb.iat[i,3]
            course_ID=st.split(":")
            #list of course IDs for that batch
            break
    for i in range(len(course_ID)):
        courseID=course_ID[i]

        #taking all courses one by one that are in batch
        for j in range(len(dfc)):
            if dfc.iat[j,0]==courseID:#searching for the course
                course.append(dfc.iat[j,1])

                m = str(dfc.iat[j, 2]).split("-")

                #student-marks pair list for that course
                for k in range(len (m)):
                    sm=m[k].split(":")

                    if sm[0]==id:
                        marks.append(int(sm[1]))
    ...
    f.write("ROLL NUMBER-" + str(roll)+'\n')
    print("\n")
    for i in range(len(course)):
        total+=marks[i]
        list1=grade(marks[i])#list
        f.write("Course="+course[i]+"("+course_ID[i]+")"+"\t"+"marks="+str(marks[i])+'\t'+"Grade="+list1[0]+"\t\t"+"Pass/Fail status="+list1[1]+'\n')

    total=float(total)/ len(marks)
    list2=grade(total)
    print("\n")
    
    f.write("OVER ALL GRADE="+str(list2[0])+'\n')
    f.write("PASS STATUS=" + str(list2[1])+'\n')
    f.close()
