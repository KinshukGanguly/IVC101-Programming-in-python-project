    ...
    for i in range(len(students)):
        ...
        for j in range(len(dfs)):
            if dfs.iat[j, 0] == student_ID:
                print("Student name=" + dfs.iat[j, 1] + "\n"+"Student_Id="+student_ID+"\n"+"Roll number="+str(dfs.iat[j, 2])+"\n")
                break
...
def view_stud_performance():
    ...
    for i in range(len(dfb)):
        if dfb.iat[i,0]==id:
            ...
            stud_list=dfb.iat[i,4].split(":")
            course_list=dfb.iat[i,3].split(":")
            break

    percent=[]
    st_name=[]
    for i in range(len(stud_list)):
        st_ID=str(stud_list[i])
        name=''
        roll=0
        course_ID=""
        marks=[]
        ...
        percentage=round(float(sum(marks))/len(marks))
        percent.append(percentage)
        print("PERCENTAGE OBTAINED="+str(percentage)+"\n")

    pyplot.pie(percent,labels=st_name)
    pyplot.show()
